classdef graphViz4Matlab < handle
    %GRAPHVIZ4MATLAB Visualize a graph in a Matlab figure window by
    % specifying an adjacency matrix and optionally node labels,
    % descriptions, colors and the the layout algorithm. The best layout
    % algorithms require that graphViz be installed, available free at
    % <http://www.graphviz.org>.
    %
    % Matthew Dunham University of British Columbia Last Updated: April 24,
    % 2010 Requires Matlab version 2008a or newer.
    %
    % Syntax (see more examples below):
    % graphViz4Matlab('-adjMat',adj,'-nodeLabels',labels,'-nodeColors',colo
    % rs);
    %
    % Self loops are removed and not represented on the graph.
    %
    % Once the graph is displayed, there are several operations you can
    % perform with the mouse. (1) Move a single node around. (2) Draw a
    % mouse box around several nodes and move them all together. (3) Enter
    % a description for a node by double clicking it. (4) Shade the node by
    % right clicking it (5) Display a node's properties on the console by
    % shift clicking it. (6) Increase or decrease the font size (7)
    % Increase or decrease the node size (8) Tighten the axes and relax the
    % square aspect ratio. (9) Ask the current layout algorithm to layout
    % the nodes as though the
    %     arrows were pointing the other way. This only affects some of the
    %     layouts.
    % (10) Change the layout algorithm and refresh the graph
    %
    % Additionally, any operation you could do with a regular Matlab figure
    % can be done here, e.g. annotating or saving as a pdf.
    %
    % Options are specified via name value pairs in any order. [] denote
    % defaults.
    %
    % '-adjMat'           [example matrix] The adjacency matrix
    %
    % '-layout'            [Gvizlayout if graphViz installed, else
    % Gridlayout]
    %                      A layout object, i.e. Gvizlayout | Gridlayout |
    %                      Circlelayout
    %                     (See knownLayouts Property)
    %
    % '-nodeLabels'        ['1':'n'] A cell array of labels for the nodes
    %
    % '-nodeDescriptions'  [{}] Longer descriptions for the nodes,
    % displayed when
    %                      double clicking on a node.
    %
    % '-nodeColors'        ['c'] A cell array or n-by-3 matrix specifying
    % colors
    %                      for the nodes. If fewer colors than nodes are
    %                      specified, the specified colors are reused and
    %                      cycled through.
    %
    ...
    %                         list only the n < numel(edges) edges you want
    %                         to color. If you do not label the nodes,
    %                         graphViz4Matlab uses '1','2','3', etc, in
    %                         which case use these. You can specify the
    %                         text 'all' in place of toName, to mean all
    %                         nodes, i.e. {fromName,'all',color}
    %
    %
    ...
    %                      can pass in a custom function handle. The
    %                      function gets passed the node's label.
    % Examples:
    %
    % adj = rand(5,5) > 0.8; labels =
    % {'First','Second','Third','Fourth','Fifth'}; colors = {'g','b'};    %
    % will cycle through s =
    % graphViz4Matlab('-adjMat',adj,'-nodeLabels',labels,'-nodeColors',colo
    % rs); freeze(s); % convert to an image
    %
    % If you are only specifying an adjacency matrix, you can omit the
    % '-adjMat' name as in graphViz4Matlab(adj).
    %
    % Calling graphViz4Matlab without any parameters displays an example
    % graph.
    %
    ...
    methods
        ...
        function redraw(obj)
            % Redraw the graph. (You could also call draw() again but then
            % the window is recreated as well and it doesn't look as nice).
            if(~obj.isvisible)
                obj.draw();
                return;
            end
        ...
        function flip(obj,varargin)
            % Have the layout algorithms layout the graph as though the
            % arrows were pointing in the opposite direction. The node
            % connectivity remains the same and if node one pointed to node
            % 2 before, it still does after. This is useful for tree
            % layout, for example to turn the tree on its head. Calling it
            % twice flips it back.
        ...
        function erase(obj)
            % Erase the graph but maintain the state so that it can be
            % redrawn.
            if(obj.isvisible)
                obj.clearGroupSelection();
                delete(obj.fig);
        ...
        function nodeSelected(obj,node)
            % This function is called by nodes when they are selected by
            % the mouse. It should not be called manually.
        ...
        function addLayout(obj,layout)
            % Let the graph know about a new layout you have created so
            % that it will be available via a toolbar button. The layout
            % object must be a descendant of the Abstractlayout class. This
            % method does not have to be called for existing layouts, nor
            % does it need to be called if you passed the new layout to the
            % constructor or to the setLayout() method. It will not add two
            % layouts with the same name property.
    ...
    methods (Access = 'private')
        function setNodeSizeInternal(obj, newSize, varargin)
            % setNodeSizeInternal(S) sets nodes to size S (but not smaller
            % than a calculated minimum).
            obj.clearGroupSelection();
            s = max(newSize,obj.minNodeSize);
        ...
        function setCallbacks(obj)
            % Set the callback functions for the figure, i.e. functions
            % that will be called when the user performs various actions.
        ...
        function addNodes(obj)
            % Add all of the nodes to the graph structure, but don't
            % display them yet.
            obj.nodeArray = [];
            for i=1:obj.nnodes
        ...
        function addEdges(obj)
            % Add all of the edges to the graph structure, but don't
            % display them yet.
        ...
        function displayEdges(obj,indices)
            % Display or refresh the specified edges. If none specified,
            % all are refreshed. Currently only works with round nodes.
            figure(obj.fig);
            if(nargin < 2)
        ...
        function [X,Y,Xarrow,Yarrow] = calcPositions(obj,edge)
            % Helper function for displayEdges() - calculates edge and
            % arrow start and end positions in data units.
            ...
            % dx:  x-distance from node1 center to perimeter in direction
            % of node2 dy:  y-distance from node1 center to perimeter in
            % direction of node2 ddx: x-distance from node1 perimeter to
            % base of arrow head ddy: y-distance from node1 perimeter to
            % base of arrow head dpx: x-offset away from edge in
            % perpendicular direction, for arrow head dpy: y-offset away
            % from edge in perpendicular direction, for arrow head
        ...
        function groupSelectionStage1(obj)
            ...
            % button has been pressed somewhere on the axes, (or on a
            % node).
    ...
    methods
        ...
        function setNodeSize(obj, newSize)
            % setNodeSize(S) sets nodes to size S (but not smaller than a
            % calculated minimum).
            obj.setNodeSizeInternal(newSize);
        end
        
        function shrinkNodes(obj,varargin)
            % shrinkNodes(S) sets nodes to size s. shrinkNodes() shrinks
            % the nodes to 95% of their original size, (but not smaller
            % than a calculated minimum).
            obj.setNodeSizeInternal(0.8*obj.nodeArray(1).width);
        end
        ...
        function growNodes(obj,varargin)
            ...
            obj.setNodeSizeInternal(obj.nodeArray(1).width/0.8);
        end
        ...
        function setNodePositions(obj, XY)
            % Programmatically set the node positions XY(i, 1) is the
            % xposition of node i, XY(i, 2) is the yposition.
            for i=1:obj.nnodes
